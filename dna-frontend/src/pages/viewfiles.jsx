...
function ViewFiles() {
  ...
  const doDel = async (fid) => {
    ...
    setDelId(fid);
    try { await deleteFile(fid); setTimeout(()=>{setFiles(p=>p.filter(f=>f.file_id!==fid));setDelId(null)},300); }
      ...
      <div className="page-content">
        ...
        {files.length===0&&!error?(
          ...
          <div className="card" style={{padding:0,overflow:"hidden"}}>
            ...
            {files.map((f,i)=>(
              <div key={f.file_id} style={{display:"grid",gridTemplateColumns:"44px 1fr 1fr 100px auto",gap:"14px",padding:"12px 18px",borderBottom:i<files.length-1?"1px solid #2a2440":"none",alignItems:"center",opacity:delId===f.file_id?0:1,transition:"opacity 0.3s,background 0.15s"}}
                ...
                <div style={{display:"flex",gap:"6px"}}>
                  {acts(f).map(a=>(
                    <button key={a.label} onClick={a.fn} style={{padding:"5px 10px",background:"transparent",border:`1px solid ${a.color}44`,borderRadius:"5px",color:a.color,fontSize:"12px",fontWeight:"600",cursor:"pointer",transition:"all 0.15s"}}
                      onMouseOver={e=>{e.target.style.background=a.color;e.target.style.color="#fff"}} onMouseOut={e=>{e.target.style.background="transparent";e.target.style.color=a.color}}>
                      {a.label}
                    </button>
                  ))}
                </div>
