...
function Register() {
  ...
  const handleRegister = async () => {
    ...
    if (form.password.length < 8) { setError("At least 8 characters"); return; }
    if (!/[A-Z]/.test(form.password)) { setError("Must contain uppercase"); return; }
  ...
  const eyeBtn = { position: "absolute", right: "12px", background: "transparent", border: "none", color: "#6b5f8a", cursor: "pointer", padding: "0" };
    ...
    <div style={{ minHeight: "100vh", background: "#0a0912", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ width: "100%", maxWidth: "400px", padding: "0 20px" }}>
        <div style={{ textAlign: "center", marginBottom: "36px" }}>
          ...
          <div style={{ fontSize: "14px", color: "#9a8fc0", marginTop: "6px", fontWeight: "500" }}>Create your account</div>
        </div>
        <div style={{ background: "#12101e", border: "1px solid #2a2440", borderRadius: "10px", padding: "28px" }}>
          ...
          <div style={{ marginBottom: "14px" }}>
            ...
            <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
              <input className="input-field" type={showPw ? "text" : "password"} name="password" placeholder="Min 8, uppercase, number, special" value={form.password} onChange={handle} style={{ paddingRight: "40px" }} />
              ...
              <div style={{ marginTop: "8px" }}>
                <div style={{ display: "flex", gap: "4px", marginBottom: "6px" }}>
                  {[form.password.length >= 8, /[A-Z]/.test(form.password), /\d/.test(form.password), /[^a-zA-Z0-9]/.test(form.password)].map((m, i) => (
                    <div key={i} style={{ flex: 1, height: "3px", borderRadius: "2px", background: m ? "#22c55e" : "#2a2440", transition: "background 0.3s" }} />
                  ))}
                </div>
                <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                  {[["8+ chars", form.password.length >= 8], ["Uppercase", /[A-Z]/.test(form.password)], ["Number", /\d/.test(form.password)], ["Special", /[^a-zA-Z0-9]/.test(form.password)]].map(([l, m]) => (
                    <span key={l} style={{ fontSize: "11px", fontWeight: "600", color: m ? "#22c55e" : "#6b5f8a" }}>{m ? "✓" : "○"} {l}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <div style={{ marginBottom: "20px" }}>
            ...
            {form.confirmPassword.length > 0 && <div style={{ fontSize: "12px", fontWeight: "600", marginTop: "6px", color: form.password === form.confirmPassword ? "#22c55e" : "#ef4444" }}>{form.password === form.confirmPassword ? "✓ Passwords match" : "✗ Passwords do not match"}</div>}
          </div>
          ...
          {success && <div style={{ fontSize: "13px", fontWeight: "600", color: "#22c55e", marginBottom: "16px", padding: "10px 14px", background: "#0a1a12", borderRadius: "6px", border: "1px solid rgba(34,197,94,0.3)" }}>✓ {success}</div>}
          <button className="btn-primary" style={{ width: "100%" }} onClick={handleRegister}>Create account</button>
          <div style={{ marginTop: "20px", textAlign: "center", fontSize: "13px", fontWeight: "600" }}>
